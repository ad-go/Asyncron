    ...
    document.getElementById('user-save-btn').addEventListener('click', function () {
        ...
        fetch(id ? (updateBase + '/' + id) : createUrl, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(function (result) {
                ...
                // Two distinct shapes come back from this same endpoint pair:
                // per-field validation failures as 'errors' (an object, see
                // UsersController::create()/update()'s own
                // $this->validator->getErrors()), and a single non-field
                // rejection as 'error' (a plain string - e.g. update()'s own
                // "cannot remove your own superadmin" guard). Only the first
                // was ever read here - the second silently fell through to
                // the generic saveFailedLabel ("Could not save - check the
                // fields below") with nothing to actually check, since no
                // field was ever marked invalid for that case. Found live
                // 2026-08-21 reproducing exactly that: a superadmin unticking
                // their own checkbox got the generic message instead of the
                // real, specific reason already sitting in the response.
                var errors = result.data && result.data.errors ? Object.values(result.data.errors) : [];
                var message = errors.length ? errors.join(' ') : ((result.data && result.data.error) || saveFailedLabel);
