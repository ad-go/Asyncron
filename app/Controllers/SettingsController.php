...
class SettingsController extends BaseController
{
    ...
    // Encrypts an export payload (exportSettings()'s own $payload - RSA
    // private key, secretToken and every node's plaintext credentials, all
    // in one JSON blob) with a password the user types at export time, so
    // the downloaded file is safe to move around (email, a shared drive,
    // ...) without carrying the whole mesh's credentials in the clear. AES-
    // 256-GCM via ext-openssl - already a hard dependency of this project
    // (Config\Cluster::$signingPrivateKey's own RSA keypairs are generated
    // through the very same extension), so this needs nothing new
    // installed. PBKDF2-SHA256 (ext-hash, always compiled into PHP) turns
    // the password into a 256-bit key; a fresh random salt AND a fresh
    // random 96-bit GCM nonce every call (never reused - reusing a nonce
    // under the same key breaks GCM's confidentiality guarantee) means two
    // exports of the identical payload with the identical password still
    // produce completely different ciphertext. GCM's own 128-bit auth tag
    // (not a separate HMAC) is what lets decryptExportEnvelope() below
    // detect a wrong password OR a tampered/corrupted file - openssl_decrypt
    // just returns false the moment the tag doesn't verify, before any
    // plaintext is trusted.
    private function encryptExportPayload(array $payload, string $password): array
    {
        $iterations = 100000;
    ...
    // The other half of encryptExportPayload() above - importSettings()/
    // importCluster() both call this the instant they see an uploaded
    // file's top-level "encrypted": true (added by this same pair, so any
    // file either export flow produced round-trips through whichever import
    // flow makes sense for the importing node's own current state, exactly
    // like an unencrypted export already does). Returns null - never throws
    // - for EVERY failure case (malformed envelope, wrong password, a file
    // that's been tampered with after export): the caller can't tell which
    // happened, on purpose, so an import failure never leaks whether a
    // guessed password was "close".
    private function decryptExportEnvelope(array $envelope, string $password): ?array
    {
        if ($password === '') {
            return null;
        }
    ...
    public function importSettings(): ResponseInterface
    {
        ...
        if (($decoded['encrypted'] ?? false) === true) {
            $decoded = $this->decryptExportEnvelope($decoded, (string) $this->request->getPost('password'));
            if ($decoded === null) {
