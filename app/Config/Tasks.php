...
class Tasks extends BaseTasks
{
    ...
    public function init(Scheduler $schedule)
    {
        ...
        if (class_exists(\AdGo\Cluster\Cluster::class)) {
            ...
            // Real SSH connectivity, same everyMinute() cadence as sync/
            // pull above, not time-drift's hourly one - whether a node is
            // reachable over SSH right now is worth knowing within a
            // minute of it changing.
            $schedule->command('cluster:ssh-check')->everyMinute();
        }
    }
}
