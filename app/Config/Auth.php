...
{
    /**
     * Login mode, overridable with auth.loginIdentifier in .env:
     * email, username, or both.
     */
    public string $loginIdentifier = 'email';

    public function __construct()
    {
        parent::__construct();

        $this->loginIdentifier = in_array($this->loginIdentifier, ['email', 'username', 'both'], true)
            ? $this->loginIdentifier
            : 'email';
        $this->validFields = $this->loginIdentifier === 'both'
            ? ['email', 'username']
            : [$this->loginIdentifier];
        // Not '\App\Views\Auth\login' (ShieldAuth's own stock override
        // point) - a leftover from when this repo WAS the app root, with
        // its own app/Views/Auth/login.php. Once this became a `composer
        // require`-able library the real view moved to
        // src/UI/Views/Auth/login.php (AdGo\Cluster\UI\Views\Auth\login -
        // see composer.json's own psr4 AdGo\Cluster\UI\ => src/UI/), the
        // same "moved once this became a package" migration RouteRegistrar
        // itself documents for routes - this one line was missed. Found
        // live 2026-09-01 writing this suite's own SessionAuthHttpTest: a
        // fresh checkout's GET /login 500s ("Invalid file:
        // \App\Views\Auth\login.php") until this points at the view's
        // actual current location.
        $this->views['login'] = '\AdGo\Cluster\UI\Views\Auth\login';
        $this->redirects['login'] = 'dashboard';
        $this->redirects['logout'] = 'login';
    }
    /*
     * ////////////////////////////////////////////////////////////////////
     * AUTHENTICATION
