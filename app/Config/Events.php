...
Events::on('login', static function ($user): void {
    ...
    // Real SSH connectivity check, queued (not run inline) right after a
    // successful login - see README "How it works", SSH connectivity
    // checks & clock drift.
    if (class_exists(\AdGo\Cluster\Cluster::class)) {
        service('queue')->push((new \AdGo\Cluster\Cluster())->queueName(), 'cluster-ssh-connectivity-check', []);
    }
});