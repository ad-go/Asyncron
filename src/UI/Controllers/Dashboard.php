...
class Dashboard extends BaseController
{
    public function index(): string
    {
        ...
        // Not the bare 'dashboard' - see AuthController::logoutView()'s
        // own docblock for why a name with no backslash in it can never
        // resolve outside the consuming app's own (empty) app/Views/.
        return view($isSuperadmin ? '\AdGo\Cluster\UI\Views\dashboard' : '\AdGo\Cluster\UI\Views\dashboard-user', [
            'user'        => auth()->user(),
            ...
            // The 'user' group's own simplified view - two cards, not the
            // full cluster topology - see userSummary()'s own docblock.
            'userSummary' => $isSuperadmin ? null : $this->userSummary(),
    ...
    // The 'user' group's own dashboard: just the two numbers that matter
    // to them (files synced, production-DB sync activity) - never the
    // full peer topology/URLs/health graph networkInfo() exposes, which
    // stays superadmin-only. Reuses Cluster::dashboardSummary() (file
    // sync - identical to what networkInfo() already feeds the
    // superadmin summary strip) and dbSyncDashboardSummary() (generic
    // DB-sync activity, which since the Phase-1 production migration -
    // see jaunty-hugging-pascal.md - is now dominated by real business
    // rows rather than just this package's own Users/Settings sync).
    private function userSummary(): ?array
    {
        ...
        return [
            ...
            'dbLastActivityAgo' => $db['recentLog'] !== [] ? $this->timeAgo((int) $db['recentLog'][0]['time']) : null,
        ];
    }
