...
class AuthController extends BaseController
{
    ...
    private function fastestNodeRedirect(): ?RedirectResponse
    {
        ...
        $profile = model(\AdGo\Cluster\UI\Models\UserProfileModel::class)->forUser($user->id);
    ...
    {
        // Not the bare 'Auth/logout' - see loginView()'s own
        // config('Auth')->views['login'] for why: CI4's view() helper only
        // ever searches APPPATH/Views/ for a name with no backslash in it
        // (FileLocator::legacyLocate() has no cross-namespace fallback at
        // all, unlike locateFile()'s own namespaced-array search branch,
        // which only ever runs for a string containing '\\'). Found live
        // 2026-09-01: every view() call in this package's own UI
        // controllers using a bare/slash-only name 500'd on every real
        // node (h1q/beta/upz) - "Invalid file" - since the actual .php
        // lives at vendor/ad-go/asyncron/src/UI/Views/, never in the
        // consuming app's own (empty) app/Views/. Only ever caught by
        // this package's own test suite reaching a logged-in page, which
        // it never did (see tests/UI/SessionAuthHttpTest's own docblock).
        return view('\AdGo\Cluster\UI\Views\Auth\logout');
    }
