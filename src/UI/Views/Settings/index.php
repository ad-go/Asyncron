    ...
    <div class="col-12 col-lg-8">
        <div class="card">
            <div class="card-body compact-form" id="settings-form"
                 data-endpoint="<?= url_to('settings.update') ?>"
                 data-logo-endpoint="<?= url_to('settings.uploadLogo') ?>"
                 data-logo-delete-endpoint="<?= url_to('settings.deleteLogo') ?>">
                ...
                <div class="row">
                    ...
                    <div class="col-6 col-md-3 mb-2"><label class="form-label"><?= lang('App.themeColor') ?></label>
                        <select class="form-select form-select-sm" data-field="themeColor">
                            <?php foreach (\AdGo\Cluster\UI\Controllers\SettingsController::THEME_COLORS as $color) : ?>
            ...
            <div class="card-header">
                ...
                <div class="card-actions ms-auto">
                    <span class="btn btn-icon btn-sm" id="settings-restart-cluster-btn"
                          data-restart-endpoint="<?= url_to('settings.restartCluster') ?>"
            ...
            <div class="card-footer">
                <label class="form-check form-switch mb-0">
                    <input class="form-check-input" type="checkbox" id="settings-sync-toggle"
                           data-endpoint="<?= url_to('settings.updateSettingsSync') ?>"
                ...
                <!-- Read-only status for Config\Cluster::$requireSignedAuth -
                     ...
                     deliberately .env-only, same as every other per-node
                     cluster switch on this page. Rendered on first load
                     only, not kept live by settings-node-status.js's 5s
                     poll - a peer gaining/losing a key is rare enough that
                     a manual reload is an acceptable way to see it update. -->
                <div class="small mt-2">
                    <?php if ($signedAuthRequired) : ?>
                        <span class="text-success"><?= lang('App.signedAuthEnforced') ?></span>
                    <?php elseif ($signedAuthReady) : ?>
                        <span class="text-info"><?= lang('App.signedAuthReady') ?></span>
                    <?php else : ?>
                        <span class="text-muted"><?= lang('App.signedAuthNotReady') ?></span>
                    <?php endif ?>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-actions ms-auto" id="settings-export-import"
                     data-export-endpoint="<?= url_to('settings.exportSettings') ?>"
                     data-import-endpoint="<?= url_to('settings.importSettings') ?>"
                     data-import-strings="<?= esc(json_encode(['success' => lang('App.importSuccess'), 'failed' => lang('App.importFailed')]), 'attr') ?>"
                     data-reset-endpoint="<?= url_to('settings.resetCluster') ?>">
                    ...
                    <span id="settings-cluster-import"
                          data-import-endpoint="<?= url_to('settings.importCluster') ?>"
            ...
            <div class="table-responsive compact-form">
                ...
                <table class="table card-table table-vcenter" id="settings-cluster"
                       data-node-endpoint="<?= url_to('settings.updateNode') ?>"
                       data-database-endpoint="<?= url_to('settings.updateDatabase') ?>"
                       data-node-test-endpoint="<?= url_to('settings.testNode') ?>"
                       data-delete-endpoint="<?= url_to('settings.deleteNode') ?>"
                       data-status-url="<?= url_to('settings.nodeStatus') ?>"
                    ...
                    <tbody>
                        ...
                        <tr data-ftp-host="<?= esc($node['ftpHost']) ?>" data-ftp-port="<?= esc($node['ftpPort']) ?>" data-ftp-user="<?= esc($node['ftpUser']) ?>" data-ftp-pass="<?= esc($node['ftpPass']) ?>"
                            ...
                            <td>
                                ...
                                <select class="form-select form-select-sm" data-row="node" data-node="<?= esc($name) ?>" data-prop="protocol" data-protocol-select>
                                    <?php foreach (\AdGo\Cluster\UI\Controllers\SettingsController::NODE_PROTOCOLS as $protocol) : ?>
                        ...
                        <tr class="border-top-0"<?php foreach (\AdGo\Cluster\UI\Controllers\SettingsController::DATABASE_TYPES as $t) : ?> data-<?= $t ?>-host="<?= esc($database[$t . 'Host']) ?>" data-<?= $t ?>-port="<?= esc($database[$t . 'Port']) ?>" data-<?= $t ?>-user="<?= esc($database[$t . 'User']) ?>" data-<?= $t ?>-pass="<?= esc($database[$t . 'Pass']) ?>" data-<?= $t ?>-database="<?= esc($database[$t . 'Database']) ?>"<?php endforeach ?>>
                                ...
                                <td>
                                    <select class="form-select form-select-sm" style="width:8em;text-align:left" data-row="database" data-node="<?= esc($name) ?>" data-prop="type" data-type-select>
                                        <?php foreach (\AdGo\Cluster\UI\Controllers\SettingsController::DATABASE_TYPES as $t) : ?>
                        ...
                        <!-- Blank "add a node" row - a two-row block, same
                             ...
                             before. -->
                        <tr id="settings-add-node-row" data-add-endpoint="<?= url_to('settings.addNode') ?>"
...
<div class="modal modal-blur fade" id="conn-test-modal" tabindex="-1"
     data-test-result-endpoint="<?= url_to('settings.testResult') ?>">
