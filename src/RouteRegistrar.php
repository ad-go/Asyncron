...
class RouteRegistrar
{
    public static function register(RouteCollection $routes): void
    {
        ...
        // Node registry propagation - see Cluster::broadcastNodeUpsert()/
        // broadcastNodeDelete() and NodeRegistryController's own docblocks.
        // receive()/deleteNode() are the push side (POST, mirroring
        // cluster/files and cluster/delete-file); pull-node-registry mirrors
        // the other pull-* routes' GET+since pattern, just returning both
        // changes and deletions in one response (see NodeRegistryState's
        // own "one file, two halves" docblock for why one endpoint is
        // enough here, unlike files/deletions needing two).
        $routes->post(
            'cluster/node-registry',
            '\AdGo\Cluster\Controllers\NodeRegistryController::receive',
            ['filter' => 'cluster-auth']
        );
        $routes->post(
            'cluster/node-registry-delete',
            '\AdGo\Cluster\Controllers\NodeRegistryController::deleteNode',
            ['filter' => 'cluster-auth']
        );
        $routes->get(
            'cluster/pull-node-registry',
            '\AdGo\Cluster\Controllers\NodeRegistryController::pullRegistry',
            ['filter' => 'cluster-auth']
        );

        // Last-resort reachability check for a peer with no SSH/FTP deploy
        // credentials configured - see PingController's own docblock and
        // NodeConnectionChecker::checkApi().
        $routes->get(
            'cluster/ping',
            '\AdGo\Cluster\Controllers\PingController::ping',
            ['filter' => 'cluster-auth']
        );
