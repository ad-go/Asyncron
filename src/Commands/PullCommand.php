...
class PullCommand extends BaseCommand
{
    ...
    private function loop(ClusterConfig $config, Cluster $cluster, array $peers): void
    {
        ...
        do {
            [$applied, $downloaded, $deleted, $dbRows, $tests, $registry, $errors] = $this->pass($config, $cluster, $peers);
            $totalApplied    += $applied;
            $totalDownloaded += $downloaded;
            $totalDeleted    += $deleted;
            $totalDbRows     += $dbRows;
            $totalTests      += $tests;
            $totalRegistry   += $registry;
            $totalErrors     += $errors;
            $passes++;
            ...
            if ($applied > 0 || $downloaded > 0 || $deleted > 0 || $dbRows > 0 || $tests > 0 || $registry > 0 || $errors > 0) {
                CLI::write(
                    sprintf('cluster:pull: pass %d - %d invalidation(s), %d file(s), %d deletion(s), %d db row(s), %d test(s) relayed, %d registry change(s), %d error(s).', $passes, $applied, $downloaded, $deleted, $dbRows, $tests, $registry, $errors),
        ...
        CLI::write(
            sprintf(
                'cluster:pull: done - %d pass(es), %d invalidation(s) applied, %d file(s) downloaded, %d file(s) deleted, %d db row(s) applied, %d test(s) relayed, %d registry change(s) applied, %d error(s) total, across %d peer(s).',
                $passes,
                $totalApplied,
                $totalDownloaded,
                $totalDeleted,
     ...
     * @param array<string, array{baseURL: string, type: string}> $peers
     *
     * @return array{0: int, 1: int, 2: int, 3: int, 4: int, 5: int, 6: int} [invalidationsApplied, filesDownloaded, filesDeleted, dbRowsApplied, testsRelayed, registryApplied, peerErrors]
     */
    private function pass(ClusterConfig $config, Cluster $cluster, array $peers): array
    {
        ...
        foreach ($peers as $peerName => $node) {
            ...
            // Own try/catch too - see the comment above for why a
            // failure here (an older ad-go/cluster peer with no
            // pull-node-registry endpoint yet, say) must never count
            // against this peer's file/invalidation/db-sync health.
            try {
                $registryApplied += $sync->pullNodeRegistry($client, $cluster, $since);
            } catch (Throwable $e) {
                CLI::write("cluster:pull: $peerName registry-pull failed - " . $e->getMessage(), 'yellow');
            }
        }

        return [$invalidationsApplied, $filesDownloaded, $filesDeleted, $dbRowsApplied, $testsRelayed, $registryApplied, $errors];
    }
