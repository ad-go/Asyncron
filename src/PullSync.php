...
class PullSync
{
    ...
    public function pullNodeRegistry(CURLRequest $client, Cluster $cluster, int $since): int
    {
        $response = $client->get('cluster/pull-node-registry', [
            'headers' => ['Authorization' => $cluster->authHeader()],
            'query'   => ['since' => $since],
        ]);

        if ($response->getStatusCode() !== 200) {
            throw new RuntimeException('pull-node-registry HTTP ' . $response->getStatusCode());
        }

        $body = json_decode($response->getBody(), true);

        return $this->applyNodeRegistrySnapshot(
            $cluster,
            is_array($body['changes'] ?? null) ? $body['changes'] : [],
            is_array($body['deletions'] ?? null) ? $body['deletions'] : []
        );
    }

    /**
     * Post-fetch half of pullNodeRegistry() above - see applyInvalidations()'s
     * own docblock for why this is factored out (also what
     * LongPollCommand::round() calls directly against an already-bundled
     * long-poll response, same as every other applyX() here).
     *
     * @param array<string, array{baseURL: string, type: string, publicKey: string}> $changes
     * @param array<string, int>                                                     $deletions name => deletedAt
     */
    public function applyNodeRegistrySnapshot(Cluster $cluster, array $changes, array $deletions): int
    {
        $applied = 0;
        foreach ($changes as $name => $entry) {
            $result = $cluster->applyIncomingNodeUpsert((string) $name, (array) $entry);
            if ($result['applied']) {
                $applied++;
            }
        }
        foreach ($deletions as $name => $deletedAt) {
            $result = $cluster->applyIncomingNodeDelete((string) $name, (int) $deletedAt);
            if ($result['applied']) {
                $applied++;
            }
        }
