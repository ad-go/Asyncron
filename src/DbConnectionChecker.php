 ...
 * config exists" idea as NodeConnectionChecker, for the Databases table
 * instead of the Nodes table's deploy credentials.
 *
 ...
 *
 * Triggered from cluster-ui's Cluster table: clicking a node's name badge
 * opens a modal that calls SettingsController::testNode(), which tests
 * this alongside NodeConnectionChecker together - see CapabilityChecker::
 * runCombined()'s own docblock.
 */
class DbConnectionChecker
{
    ...
    // Postgre, oci8 -> OCI8). Public - SettingsController::
    // syncOwnDatabaseEnv() reuses this exact mapping when writing
    // database.production.DBDriver into .env, rather than keeping a
    // second copy that could drift from this one.
    public const DRIVER_CLASS = [
        'mysql'    => 'MySQLi',
        'postgres' => 'Postgre',
        'sqlite3'  => 'SQLite3',
        'oci8'     => 'OCI8',
    ...
    public function checkParams(array $params): array
    {
        ...
        // CodeIgniter's own MySQLi Connection::connect() does
        // `$this->hostname[0] === '/'` with no length check first - an
        // empty hostname (no host configured for this node/driver at
        // all, the common case for a fresh SQLite-only install) crashes
        // with "Uninitialized string offset 0" INSIDE the framework's
        // own code, one level below where this method's try/catch below
        // can cleanly absorb it (found live 2026-08-23, reproduced
        // identically on 3 separate nodes - every 'test connection'
        // against a node with no MySQL configured 500'd instead of
        // reporting a clean failure). sqlite3 has no hostname concept at
        // all, so it's exempt from this guard.
        $host = (string) ($params['host'] ?? '');
        if ($host === '' && $type !== 'sqlite3') {
            return [
                'ok'    => false,
                'error' => "No {$type} host configured for this node.",
                'ms'    => 0.0,
            ];
        }
        ...
        $config = [
            ...
            // Explicit, not left to whatever the driver/OS defaults to - a
            // black-holed host (firewalled, wrong port, unreachable NAT
            // target) should fail this check in single-digit seconds, same
            // ballpark as every other checker in this file's family
            // (NodeConnectionChecker's own SSH/FTP checks: 10s). CI4's own
            // MySQLi driver already hardcodes MYSQLI_OPT_CONNECT_TIMEOUT=10
            // unconditionally (see its Connection::connect()), so this is a
            // no-op there; BaseConnection::__construct() only assigns a
            // config key onto a property that actually exists on the target
            // driver's connection class (Postgre has $connect_timeout -
            // this fixes it there), so it's always safe to pass regardless
            // of which driver ends up handling it.
            'connect_timeout' => 10,
        ];
