...
class Cluster
{
    ...
    public function requireSignedAuth(): bool
    {
        return $this->config->requireSignedAuth;
    }

    // Base64-of-PEM, same shape authHeader()/verifyAuthHeader() already
    // read/write - exposed as a getter (mirroring secretToken() above) so
    // SettingsController::exportSettings() can include THIS node's own key
    // in its export, the same way it already does for secretToken.
    public function signingPrivateKey(): string
    {
        return $this->config->signingPrivateKey;
    }
    ...
    /**
     * ONE outbound peer-request client - every peer-to-peer HTTP call in
     * this package built its own `service('curlrequest', [...], null,
     * null, false)` inline (12 call sites, found live 2026-08-21 doing
     * the actual count - the same one-liner, timeout aside, copy-pasted
     * everywhere from Commands/Jobs/Controllers to cluster-ui's own
     * SettingsController). `getShared: false` is required, not cosmetic,
     * on every one of them: queue:work processes many jobs (possibly for
     * DIFFERENT peers) in one PHP process, and CI4's shared service cache
     * would hand back the FIRST job's baseURI-bound client to every
     * later one in that same run otherwise.
     */
    public function peerClient(string $baseURL, int $timeout = 15): CURLRequest
    {
        return service('curlrequest', ['baseURI' => $baseURL, 'timeout' => $timeout], null, null, false);
    }
     ...
     *    downgrade to the weaker check. Skipped entirely - an unsigned
     *    header is rejected outright - once Config\Cluster::
     *    $requireSignedAuth is turned on; see that property's own
     *    docblock and allPeersReadyForSignedAuth() below.
     */
    public function verifyAuthHeader(string $header): bool
    {
        ...
        if ($this->config->requireSignedAuth) {
            return false;
        }
    ...
    /**
     * Readiness check for Config\Cluster::$requireSignedAuth - true only
     * when turning that flag on wouldn't lock out this node's own outgoing
     * calls (needs its own signingPrivateKey) or any currently-registered
     * peer's incoming calls (each needs a publicKey on file here). An
     * empty $nodes registry (a brand-new, not-yet-paired node) counts as
     * ready - there's no peer yet for a missing key to lock out - so this
     * alone is a necessary check before enabling the flag, not a
     * sufficient one: re-check after adding any new peer, the same way
     * you would re-check before enabling any other capability that
     * depends on the CURRENT registry contents.
     */
    public function allPeersReadyForSignedAuth(): bool
    {
     ...
     * SyncFilesCommand). 'nat' peers are never reached by THIS broadcast
     * directly (same reachability constraint as file push - see
     * publicPeers()'s own docblock) - they still catch it, just via their
     * own next cluster:pull/long-poll cycle pulling invalidations FROM a
     * public peer (see PullSync::pullInvalidations()), same as files/DB
     * rows, so the latency is a little higher (bounded by their own pull
     * cadence on top of this push), not "never".
     */
     ...
     * on purpose (pushing to one over plain HTTPS would just time out;
     * they catch up via their own Pull instead - see Commands\PullCommand/
     * Commands\LongPollCommand).
     *
    ...
    /**
     * Tells every public peer that $name's registry entry was just added or
     * changed on THIS node - the missing half found live 2026-08-21:
     * SettingsController::addNode()/deleteNode() only ever wrote cluster.nodes
     * on the ONE node the admin happened to be browsing, every other node
     * needed a hand re-import (export/import round trip) to even learn a new
     * peer existed. Same push-then-relay shape as broadcastInvalidation()
     * above: record locally (so THIS node's own future pull/long-poll
     * responses can relay it onward too, closing the mesh - see
     * NodeRegistryState's own docblock), then one queue job per public peer.
     *
     * No sessionSyncEnabled/fileSyncEnabled gate, deliberately - the
     * registry itself is what makes those two concerns reachable in the
     * first place (a node absent from the registry can't be pulled from or
     * pushed to at all), so gating this on either would be circular.
     *
     * @param array{baseURL: string, type: string, publicKey: string} $entry
     */
    public function broadcastNodeUpsert(string $name, array $entry): void
    {
        (new NodeRegistryState($this->config))->recordChange($name, $entry);

        foreach (array_keys($this->publicPeers()) as $peerName) {
            service('queue')->push($this->config->queueName, 'cluster-broadcast-node-upsert', [
                'name'  => $name,
                'entry' => $entry,
                'peer'  => $peerName,
            ]);
    ...
    /**
     * Mirror image of broadcastNodeUpsert() above - called by
     * SettingsController::deleteNode() AFTER this node's own cluster.nodes
     * already had $name removed, so publicPeers() here naturally excludes
     * the node being deleted (it's no longer in the registry to iterate).
     */
    public function broadcastNodeDelete(string $name, int $deletedAt): void
    {
        (new NodeRegistryState($this->config))->recordDeletion($name, $deletedAt);

        foreach (array_keys($this->publicPeers()) as $peerName) {
            service('queue')->push($this->config->queueName, 'cluster-broadcast-node-delete', [
                'name'      => $name,
                'deletedAt' => $deletedAt,
                'peer'      => $peerName,
            ]);
     ...
     *   avoid a broadcast storm across a full mesh (every node relaying the
     *   same already-known entry back out on every pull/long-poll cycle
     *   otherwise).
     *
     * On success, re-records the change into THIS node's own
     * NodeRegistryState - not a no-op duplicate: it's what lets the change
     * keep propagating to whoever pulls FROM this node next, without any
     * node needing to explicitly relay/forward anything (see
     * NodeRegistryState's own docblock, same reasoning as
     * applyIncomingDeletion()'s identical re-record of DeletedFiles below).
     *
     * @param array{baseURL: string, type: string, publicKey: string} $entry
     *
     * @return array{applied: bool, reason: string}
     */
    public function applyIncomingNodeUpsert(string $name, array $entry): array
    {
        if ($name === '' || $name === $this->config->thisNode) {
        ...
        $nodes[$name]  = $entry;
        if (! $this->writeNodeRegistryEnv($nodes)) {
            return ['applied' => false, 'reason' => 'could not write .env'];
        }
    ...
    /**
     * Mirror image of applyIncomingNodeUpsert() above - see that method's
     * own docblock for the shared "never touch our own identity" guard and
     * the "re-record so it keeps propagating" reasoning, both identical
     * here.
     *
     * @return array{applied: bool, reason: string}
     */
    public function applyIncomingNodeDelete(string $name, int $deletedAt): array
    {
        if ($name === '' || $name === $this->config->thisNode) {
            return ['applied' => false, 'reason' => 'refused - own identity'];
        }

        if (! array_key_exists($name, $this->config->nodes)) {
    ...
    /**
     * Native .env writer for cluster.nodes, used by applyIncomingNodeUpsert()/
     * applyIncomingNodeDelete() above - the peer-to-peer incoming-sync
     * side's counterpart to ClusterEnvWriter::writeNodes() (the admin-UI
     * side, in SettingsController's node CRUD/handshake/import flows).
     * Delegates the actual locked file write to ClusterEnvWriter::
     * writeLines() - the two used to be entirely independent copies of
     * the same fopen()+flock(LOCK_EX) logic (a real bug once: see
     * NodeRegistryEnvLockTest's own docblock for the concurrent-write
     * race that split copy caused) - but deliberately does NOT call
     * ClusterEnvWriter::writeNodes() itself, which also refreshes the
     * shared config('Cluster') singleton as a side effect: both callers
     * here already do that themselves right after this returns, against
     * whichever $this->config instance THEY were constructed with (not
     * necessarily the global singleton - see tests constructing Cluster
     * with an isolated ClusterConfig), so doing it again here would risk
     * mutating global state a test never asked to touch.
     */
    private function writeNodeRegistryEnv(array $nodes): bool
    {
        ...
        return ClusterEnvWriter::writeLines(['cluster.nodes' => $line]);
    }

    /**
     * One-shot clock-offset measurement against whatever peer $client is
     * already scoped to (Cristian's algorithm - offset/rtt/peerTime below
     * are the whole formula, see the method body) - queries cluster/time
     * on that SAME client. Every call site (TimeDriftCommand, PullCommand::pass(),
     ...
     * measureDrift() above for how $offset itself is derived and its sign
     * convention. Used right after a pull download, before the
    ...
    public function saveManifest(array $manifest): void
    {
        ...
        // system users on every node in this cluster, with no shared group
        // guaranteed between them (found live 2026-08-19 on h1q/bak/res:
        // each pair turned out to share a group in only ONE direction, not
        // both, so a matching-group + setgid scheme needs per-node tuning
        // to work at all - see 'Working conventions' in asyncron.md - and
        // even then only survives until the next full wipe+reinstall,
        // since nothing in this repo persists an OS-level chgrp/setgid
        // across one). 0666 (not 0664 + setgid) sidesteps groups
        // entirely - every state file under writable/Cluster/ is
        // internal sync/lock bookkeeping only, WRITEPATH-scoped, never
        // web-served and never holding a real credential (those live in
        // .env, not here), so world-writable here is a deliberate,
        // low-risk trade for a fix that works identically on every node's
        // hosting setup with zero manual per-node intervention, and
        // survives every future reinstall without needing to be reapplied
        // by hand. Found live 2026-08-18 on the newer sync_state.json (same
    ...
    /**
     * Atomic load-mutate-save cycle for the file manifest - loadManifest()/
     * saveManifest() above are each individually flock()'d, but a caller
     * doing load-then-save back to back (recordKnown(),
     * applyIncomingDeletion()) is NOT: two concurrent callers (the scanner
     * and the receiving HTTP controller both write this SAME manifest -
     * see recordKnown()'s own docblock) can each load a consistent
     * snapshot, mutate their own in-memory copy, then save - and whichever
     * finishes last silently overwrites the other's change. Holding ONE
     * exclusive lock across the whole read-mutate-write cycle (not two
     * separate SH-then-EX locks with a gap between them) closes that
     * window - same fix shape as writeNodeRegistryEnv()'s .env lock, just
     * JSON instead of .env.
     *
     * $mutate returns the manifest to persist; a caller that also needs to
     * report back what it decided (applyIncomingDeletion()'s applied/
     * rejected) does so via a variable captured by reference, so that
     * decision is made from the SAME fresh-under-lock read the write uses,
     * not a separately-timed loadManifest() call beforehand.
     *
     * @param callable(array<string, array{hash: string, mtime: int, size: int}>): array<string, array{hash: string, mtime: int, size: int}> $mutate
     */
    private function updateManifest(callable $mutate): void
    {
        $path = $this->manifestPath();
        $dir  = dirname($path);
        if (! is_dir($dir) && ! mkdir($dir, 0775, true) && ! is_dir($dir)) {
        ...
        $manifest = $mutate(is_array($decoded) ? $decoded : []);
        ...
        flock($handle, LOCK_UN);
        fclose($handle);
        // See saveManifest()'s own comment on this - identical cross-
        // user-write reasoning, same file.
        @chmod($path, 0666);
    }
    ...
    {
        $this->updateManifest(static function (array $manifest) use ($relativePath, $hash, $mtime, $size): array {
     ...
     * the rejecting node's own file is unchanged, so its own manifest
     * already matches it and nothing marks it "worth re-pushing" to
     * correct the stale sender. Accepted as a rare-race limitation rather
     * than building an explicit push-back path for it.
     *
    ...
    public function applyIncomingDeletion(string $relativePath, int $deletedAt): array
    {
        ...
        $result = ['applied' => false, 'reason' => 'local is newer'];

        $this->updateManifest(function (array $manifest) use ($relativePath, $deletedAt, $targetPath, &$result): array {
            $known = $manifest[$relativePath] ?? null;
            ...
            $result = ['applied' => true, 'reason' => 'deleted'];
    ...
    // - node reachability (writable/Cluster/node_connections.json via
    //   NodeConnectivityLog) - the STANDING connect-and-prove-usable check
    //   (NodeConnectivityChecker, via whichever transport - SSH/SCP, FTP/
    //   FTPS, or the cluster/ping API fallback - each peer is set up
    //   for), independent of any sync attempt ever having run
    ...
    public function peerHealthStatuses(): array
    {
        ...
        $connectivity = class_exists(NodeConnectivityLog::class)
            ? (new NodeConnectivityLog($this->config))->all()
            : [];
        ...
        foreach ($network['nodes'] as $name => $node) {
            // A 'nat' peer only ever gets PUSHED TO by a 'public' peer -
            // SyncFilesCommand queues push jobs from cluster->publicPeers()
            // only, so two 'nat' peers checking each other's fileOk here
            // NEVER see an incoming push between them, by design, not
            // because anything is broken (each independently pulls from
            // the shared public hub(s) instead). lastPushInAt !== null
            // being a hard bool made that permanently 'false' for every
            // nat<->nat pair regardless of actual health - found live
            // 2026-08-23 immediately after fixing the SSH veto, node3 and
            // node4 (both nat, same LAN, file+DB sync both genuinely fine)
            // still showed 'bad' for each other. Absence of an incoming
            // push here just means "not applicable/not seen yet", not
            // "failed" - only an actual recorded push tells us anything,
            // so this stays null (not false) until one arrives.
            $fileOk = $node['type'] === 'nat'
                ? ($node['lastPushInAt'] !== null ? true : null)
                : $node['lastSyncOk'];
            $connOk = $connectivity[$name]['ok'] ?? null;
            $dbOk   = $dbSyncOkByPeer[$name] ?? null;
            $queueFailed = $queueFailedByPeer[$name] ?? 0;

            // Node connectivity is a supplementary diagnostic signal
            // (NodeConnectivityChecker/clock-drift) - actual replication
            // (file sync, DB sync) rides entirely over HTTP regardless of
            // node type (see README "How it works": public-push /
            // NAT-pull, both HTTP). NodeConnectivityChecker deliberately
            // checks every peer combo regardless of type (its own
            // docblock), so a public<->nat pair across a firewall NEITHER
            // side can cross the other way - the direct-transport-
            // unreachable case, not a credentials problem - reports
            // $connOk === false here forever, even though file+DB sync
            // with that exact peer are both healthy. Found live
            // 2026-08-23 (back when this signal was SSH-only): this made
            // the LED red for a perfectly healthy peer while the same
            // node's own "test connection" badge (dispatchTest() - tests
            // from the right vantage point, not a direct ping) correctly
            // reported ok. $connOk === false must not veto the composite
            // by itself - only signals replication itself actually
            // depends on do that. A successful connectivity check still
            // counts as a positive signal below, and still surfaces on
            // its own in the Cluster table regardless of this composite.
            if ($fileOk === false || $dbOk === false || $queueFailed > 0) {
                $statuses[$name] = 'bad';
            } elseif ($fileOk === true || $connOk === true || $dbOk === true) {
                $statuses[$name] = 'ok';
            } else {
                $statuses[$name] = 'idle';
            }
     ...
     * measured (see Commands\TimeDriftCommand) - this method does not measure
