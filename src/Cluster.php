...
class Cluster
{
     ...
     * applyIncomingNodeDelete() above - src/ classes can't depend on
     * app/Controllers/SettingsController (app depends on src/, never the
     * reverse), so this is a second, deliberately minimal copy of that
     * controller's own writeEnvLines()/writeClusterNodesEnv() pair, kept in
     * sync by format only (name|baseURL|type[|publicKey], comma-separated -
     * see Config\Cluster::nodesFromEnv()'s own parser), not by sharing code.
     */
    private function writeNodeRegistryEnv(array $nodes): bool
    {
        ...
        $env     = preg_match($pattern, $env) === 1
            ? preg_replace($pattern, $newLine, $env, 1)
            : rtrim($env) . "\n" . $newLine . "\n";
