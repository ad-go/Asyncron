 ...
 * cluster-ui's Settings -> Cluster table's node-name badge (which tests
 * the file-sync AND database connections together - see
 * CapabilityChecker::runCombined()) actually run the check ON the target
 * node itself, using data handed over the wire (host/port/user/pass/
 * database), rather than from wherever the admin's browser session
 * happens to be connected. See DbConnectionChecker/NodeConnectionChecker's
 * own checkParams() for why this must run on the TARGET's own local
 * network position: a Databases row's host is often something like
 * 127.0.0.1, meaningful only from that specific node.
 *
 ...
 *   Latency is bounded by that pull cadence, not instant - see README
 *   "How it works" for the honest timing story (bounded by whatever
 *   crontab granularity the host allows, once a minute in the common
 *   case - not by anything in this class).
 */
class RemoteTestController extends Controller
{
    /**
     * Public-target path: run a check right here, right now, and return
     * the result directly. Body: {"kind": "database"|"node"|"combined",
     * ...params} - params match whichever checker (or, for "combined",
     * both) CapabilityChecker::run() resolves $kind to, since this just
     * hands the decoded body straight through - see that class's own
     * docblock for why dispatch lives there and not duplicated here.
     */
