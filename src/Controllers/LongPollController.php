...
class LongPollController extends Controller
{
    ...
    /**
     * @return array{invalidations: array<string,int>, files: array<string,array>, deletions: array<string,int>, dbRows: list<array>, testRequests: list<array>, registryChanges: array<string,array>, registryDeletions: array<string,int>}
     */
    private function collectPending(Cluster $cluster, string $callerName, int $since): array
    {
        ...
        return [
            ...
            // Registry changes/deletions - see NodeRegistryState's own
            // docblock. Since-filtered like every other channel here, so
            // hasAny() correctly holds the connection when nothing's
            // actually changed instead of returning instantly on a full
            // snapshot.
            'registryChanges'   => $state->changesSince($since),
            'registryDeletions' => $state->deletionsSince($since),
        ];
    }
    ...
    {
        return ['invalidations' => [], 'files' => [], 'deletions' => [], 'dbRows' => [], 'testRequests' => [], 'registryChanges' => [], 'registryDeletions' => []];
    }
    ...
    private function burstOwnQueue(): void
    {
        ...
        // block this request past its own budget. The +1s over
        // QUEUE_BURST_MAX_SECONDS is slack for the child's own -max-time
        // check to actually fire before this external bound does. Result
        // ignored - best-effort, and BoundedProcess::run() already
        // degrades to a no-op return when proc_open() itself fails.
        BoundedProcess::run($cmd, self::QUEUE_BURST_MAX_SECONDS + 1);
    }
}
