 ...
 * already uses.
 *
 * Deliberately does NOT create accounts itself: a ticket only ever
 * proves an EMAIL, never provisions one. If the target node has no local
 * Shield user for that email, the handoff fails gracefully back to that
 * node's own login page - this only actually completes end-to-end for an
 * email that already has a matching account on both nodes. DbSyncSchema's
 * own 'users' table sync (see its own docblock) is what actually gets an
 * account onto every node in the first place - every account eventually
 * qualifies once its own sync cycle has run, not just the superadmin
 * every node's own install bootstraps identically (see CI4install.php's
 * installSuperadminHere()) - a brand-new account can still hit the "no
 * local user yet" case below for the short window before its own first
...
class SsoController extends Controller
{
    ...
    public function consume(): ResponseInterface
    {
        ...
        try {
            $client   = $cluster->peerClient($node['baseURL'], 10);
        ...
        if ($localUser === null) {
            // Expected, not an error - either this account's own first
            // DB sync cycle (see DbSyncSchema) hasn't reached this node
            // yet, or sync is disabled/misconfigured here.
            return redirect()->to('/login')->with('error', $this->handoffFailedMessage());
        }
