...
class PushFileJob extends BaseJob
{
    ...
    public function process(): void
    {
        ...
        if ($localPath === null) {
            // Deleted locally before this job ran - not this job's
            // concern either way: deletion propagation is a SEPARATE
            // mechanism (SyncFilesCommand's own deletion scan -> Deleted
            // Files -> pull-deleted-files, not a content push), so there's
            // simply nothing left for THIS job (pushing file bytes) to do.
            return;
        }
        ...
        try {
            $client = $cluster->peerClient($node['baseURL'], 30);
