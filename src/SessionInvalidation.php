 ...
 *
 * Keyed by EMAIL, not the Shield user ID: every node's `users` row for
 * the SAME logical account still has a DIFFERENT node-local autoincrement
 * ID, even with DB sync now replicating the account itself (see
 * DbSyncSchema's own top docblock - this is deliberate there too, the
 * same reason every synced table keys on its own natural/business key
 * rather than the DB's own numeric ID). Email is the only identifier
 * that actually means the same thing across nodes.
 *
