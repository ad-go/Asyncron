...
class Cluster extends BaseConfig
{
     ...
     * 'nat' (behind a firewall, only ever Pulls - see Commands\PullCommand/
     * Commands\LongPollCommand for how). 'publicKey' is that peer's RSA
     * public key (see $signingPrivateKey below) - '' for a peer whose
     * registry entry hasn't been upgraded to the 4-segment form yet.
     *
     ...
     * (see Cluster::authHeader()), and (as long as $requireSignedAuth
     * below is left off) ClusterAuthFilter accepts that from ANY peer,
     * key or no key - the single shared $secretToken this was meant to
     * retire stays a fully valid credential on its own until
     * $requireSignedAuth is explicitly turned on. See that property's own
     * docblock for the retirement path.
     * .env: cluster.signingPrivateKey
     */
    public string $signingPrivateKey = '';

    /**
     * Retires the legacy bare Authorization: Bearer $secretToken scheme
     * once the whole cluster has moved to per-node signing keys -
     * ClusterAuthFilter::verifyAuthHeader() rejects an unsigned header
     * outright while this is true, instead of accepting it as a fallback.
     * Off by default: turning this on before every node genuinely has a
     * publicKey on file everywhere would lock out any node that hasn't
     * upgraded yet, so leave it off and check
     * Cluster::allPeersReadyForSignedAuth() first.
     *
     * Deliberately per-node (not something ONE node's flip enforces
     * cluster-wide): each node's own ClusterAuthFilter only ever protects
     * ITS OWN incoming endpoints, so this has to be turned on individually,
     * same as $signingPrivateKey's own per-node rollout above.
     * .env: cluster.requireSignedAuth
     */
    public bool $requireSignedAuth = false;
     ...
     *
     * Defaults to 'production' - Config\Database's own $production group,
     * the same one Settings > Cluster -> Databases (and importSettings()/
     * importCluster() via syncOwnDatabaseEnv()) already populate for THIS
     * node's business database - so whole-database sync is ON out of the
     * box the moment that group has real credentials, with no separate
     * .env edit required beyond configuring/importing Databases. Harmless
     * on a node that hasn't configured Databases yet: genericTables()
     * catches the connection failure and logs it, same as any other
     * unreachable group (see its own try/catch below). Set .env's
     * cluster.dbSyncGroup explicitly to an empty value to opt back out
     * entirely - genericTables() then returns only whatever's already
     * synced with no group-wide discovery at all, same as before this
     * feature existed - or to a different group name to sync some other
     * connection instead.
     *
    ...
    public function __construct()
    {
        ...
        if ($this->dbSyncGroup === '') {
            // env() returns null when .env has no cluster.dbSyncGroup key
            // at all (default to 'production' - see this property's own
            // docblock) vs. an empty string when the key exists but was
            // deliberately left blank (explicit opt-out, honored as-is).
            $group = env('cluster.dbSyncGroup');
            $this->dbSyncGroup = is_string($group) ? trim($group) : 'production';
        }
