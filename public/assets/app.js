(function () {
    ...
    // second AJAX call in a row 403s. Exposed on window - settings.js/
    // profile.js/users.js all use this exact same function as a .then()
    // callback in their own fetch chains; only safe to share like this
    // because none of them ever call it synchronously at top-level script
    // execution time (every call site is itself inside a later promise
    // callback), so it's always defined by the time any of them actually
    // run, regardless of this script tag's position relative to theirs.
